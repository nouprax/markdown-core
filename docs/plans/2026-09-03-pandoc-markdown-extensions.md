...
- normative prose: the Pandoc 3.11 `MANUAL.txt` blob at tag commit
  `b913622e1ff87c69ab8b1a606577122e220925cd`;
- grammar and extension registry evidence: the `Markdown.hs` and
  `Extensions.hs` blobs at that same commit;
- executable implementation: the official Pandoc 3.11 release CLI, verified
  against the official per-platform SHA-256 before first use;
- observation format: Pandoc native JSON from `--to=json`, without citeproc,
  filters, templates, defaults files, or bibliography lookup; and
- reader isolation: `markdown_strict` plus the exact extension set declared by
  each case in the input-only oracle corpus. The default `markdown` extension
  bundle is forbidden.
...
1. Add `oracle-pandoc` to `scripts/init-environment.sh`. `--install` downloads
   ...
   network access.
2. Implement one oracle adapter that reads
...
3. Define one semantic projection per target concept, not per example. It may
   ...
   cell spans.
4. Keep the corpus input-only. Product expected AST belongs in the C fixtures
...
1. Add the universal ordered `attributes` field defined by
...
2. Add `Span`, `Superscript`, `Subscript`, `Div`, `DefinitionList`,
...
3. Add the shared bibliography branch to `Cite`, `Citation`, and
...
4. Allocate C kind/field identifiers once, then update the native tree, C
...
5. Expose one independent option per public extension named by the index.
   ...
   no invented option.
6. Activate product comparison once those options and target values can be
...
1. Implement the shared attribute scanner and normalization operation once.
   ...
   storage shape.
2. Attach inline code, heading, fenced-code, link/image, bracketed-Span, and
...
3. Finalize explicit and generated heading IDs in one source-ordered registry.
   ...
   same final values.
4. Audit every existing Link/Image, Heading, Code/CodeBlock, directive, and
...
1. Recognize fenced Divs in the existing container stack. Opening attributes
...
2. Generalize the ordered-list marker operation for Pandoc style, delimiter,
...
3. Add example-list numbering and label registration as document parser state.
   Store authored labels and list starts, not a second derived ordinal on each
   item.
4. Add definition lists before paragraph fallback using bounded lookahead for a
...
1. Generalize the current table construction path so inherited pipe, simple,
...
2. Implement captions as an attachment candidate claimed by the nearest
   complete eligible table. Do not emit a caption paragraph and later repair
   the AST.
3. For simple and multiline forms, derive alignment and optional relative
...
4. For grid tables, maintain a row-width occupancy array. Store a spanning cell
   ...
   boundaries.
5. Decide table, thematic-break, Setext-heading, fenced-code, definition-list,
...
1. Add package-owned fixtures mapped to every normative Pandoc module. Cover
...
2. Extend the shared canonical AST corpus with every new kind, enum case,
   nullable field, universal attributes state, citation branch, list form,
   definition body shape, and table span arrangement.
3. Remove each parity gap in the implementation commit that closes it. Any
...
4. Run native correctness/conformance, Swift macOS, Kotlin JVM, ES Node and
...
Release notes must list the exact supported extension names, the absence of a
monolithic Pandoc preset, the universal attributes and shared citation model,
the exact 3.11 source/runner pin, and every public AST migration.
