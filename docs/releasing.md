...
one protected Git commit and one version in `VERSION`. The first coordinated
release is `1.0.2`; npm `1.0.0` was the trusted-publisher bootstrap described
below. The protected `v1.0.1` attempt failed before artifact construction or
publication and remains immutable as a failed release attempt. The repository
does not inherit old cmark-gfm tags, and it does not promise C binary ABI
compatibility between releases. Consumers must rebuild the C engine and
...
The npm package is public `@nouprax/es-markdown-core`. Its initial public
`1.0.0` bootstrap publish completed on 2026-07-15 through interactive CLI web
authentication and security-key 2FA. Its GitHub Actions trusted publisher is
configured with these exact values:
...
The bootstrap CLI session/token was revoked immediately after configuration;
`npm whoami` was verified to return `ENEEDAUTH`. Package publishing access now
requires 2FA and disallows traditional tokens. No npm write token belongs in
GitHub. The `1.0.2` release must run one OIDC publication and verify the
provenance attestation on npm. npm requires a GitHub-hosted runner plus `contents: read`
and `id-token: write`; the release job grants exactly those permissions. See
npm's current [trusted publisher procedure](https://docs.npmjs.com/trusted-publishers/).
