Why use `markdown_core` and not X?
==========================
...
than `markdown_core` in our benchmarks (0.21s vs. 0.29s).  But both
...
DOS attacks," but there are some chinks in the armor:
    ...
    % time python -c 'print(("[" * 50000) + "a" + ("]" * 50000))' | markdown_core
    ...
    user 0m0.073s
...
`markdown_core` is about six times faster.

`kramdown`
----------

`markdown_core` is about a hundred times faster.
