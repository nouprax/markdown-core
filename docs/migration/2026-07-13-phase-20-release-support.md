...
状态：待 Phase 19 质量门禁完成后实施。Maven Central namespace 所有权已验证；版本、
registry、签名、OIDC、provenance、release environment、dry-run 和正式发布链仍待完成。
...
- [ ] 对齐 C、SwiftPM、Maven 和 npm 版本，并拒绝任一 artifact version 漂移。
...
- [ ] 从 `swift package archive-source` 解包到 repo 外目录，验证 root canonical contract、SwiftPM build-tool plugin、provider conformance 和只依赖 `MarkdownCore` product 的独立 consumer；source archive 必须保留测试合同，但 consumer build 不得执行测试插件或携带 derived fixture。
- [x] Maven Central `com.nouprax` namespace 已通过 `nouprax.com` DNS TXT 完成所有权验证。
- [ ] 验证 Maven publication coordinate 为 `com.nouprax:kotlin-markdown-core:<version>`，并确保 KMP root、JVM、Android 和所有 Native target publications 在同一 Central deployment 中齐全。
- [ ] 验证 POM、Gradle Module Metadata、sources/javadoc artifacts、checksums、signatures、target-specific coordinates 与 native payload 完整一致。
...
- [ ] 创建有过期时间的 Maven Central Portal user token，并通过受保护 `release` GitHub environment 提供 `MAVEN_CENTRAL_USERNAME`/`MAVEN_CENTRAL_PASSWORD`。
- [ ] 创建带 passphrase 的 PGP signing key、发布 public key，并通过受保护 environment 提供 `MAVEN_SIGNING_KEY`/`MAVEN_SIGNING_PASSWORD`；记录离线备份、轮换、撤销与泄漏响应。
...
- [ ] 为 npm publish job 配置最小 `id-token: write`/`contents: read`，验证 provenance 并禁用传统 token publishing。
- [ ] 创建受保护 `release` environment，配置 required reviewer、tag/branch restrictions 和 Maven-only secrets；GitHub Release job 仅在必要时使用 workflow-provided `GITHUB_TOKEN` 的 `contents: write`。
- [ ] 添加完全不读取 release secrets 的 release dry-run，验证 artifact contents、versions、metadata、checksums、signatures 和 provenance inputs。
- [ ] 创建 `docs/releasing.md`，记录认证、secret names、轮换、撤销、trusted publishing、offline signing-key backup、泄漏响应、changelog、release notes 和发布前检查。
...
- [ ] 验证 C install、Maven、npm 与 Swift compiled public product 不包含 shared spec corpus、未预期 public headers/symbols、renderer、private extensions library、native handles 或 runtime implementation files；SwiftPM source archive 必须保留 repo-owned tests/specs，且外部 consumer 只构建 product targets。
