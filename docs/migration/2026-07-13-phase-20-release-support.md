...
Gradle/KMP clean-import 均已落地；协调版本 `1.0.1` 的首次 OIDC provenance 和完整正式
release 仍待完成。
...
- [x] 以 npm `1.0.0` bootstrap 建立新 release lineage，并准备首个受保护 tag 与四端协调
...
- 协调版本提升到 `1.0.1` 后，本地 host release dry-run 再次通过：C 与 Swift artifacts、
...
- 最终单根 release commit 的 GitHub
  ...
  clean-checkout checks 均已绿色；证据覆盖 Phase 19 required gates、四端
  build/conformance/consumer/package/security matrix，以及同一 commit 的 artifact staging、
  一次性 dry-run signing、checksums、provenance inputs 与 fail-closed release dry-run gate。
