...
- development-branch `push` 仍运行同一完整矩阵，但其汇总 check 名为
  `Development branch gates`。同一 SHA 随后创建 PR 时，SHA concurrency 会取消冗余的
  push run；即使该 run 的 `always()` 汇总因依赖被取消而失败，也不会发布或污染 ruleset
  所要求的 `Required gates` context。
...
同一验证 PR 还暴露了 push/PR SHA 去重的 context 冲突：被取消的 push run 仍运行
`always()` 汇总，并留下一个失败的 `Required gates`，与 PR run 的成功同名 check 同时被
