...
| direct, reference, and autolink link and image | `Link`, `Embedded`                | immediately after the occurrence, or inherited from its definition    |
...
autolink attaches to the resulting `Link` or `Embedded`; a shortcut reference
...
│   └── Embedded scope=3:1..3:40 anchor="hero" attributes={.wide width="50%"} dest=url("foo.jpg") title=null dimensions=null children=1
...
Embedded `width` and `height` assignments are records stored verbatim,
`width=50%` and `height=2in` alike; the parser validates no unit. The typed
`Embedded.dimensions: Dimensions?` value is populated only by the
