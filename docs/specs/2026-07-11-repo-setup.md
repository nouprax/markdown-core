...
- 新项目的 release lineage 由 npm `1.0.0` bootstrap 建立，首个受保护 tag 与四端协调
  release 为 `1.0.1`。
- 新 repo 不迁移、不保留、不归档原 `cmark-gfm` 的旧 tags。
- 新 repo 的 tag namespace 只包含 Markdown Core 自身的 releases，从首个协调版本
  `v1.0.1` 开始。
...
- `1.0.1` 初始 minimum deployment targets 为 iOS 18 与 macOS 15，从而覆盖 iOS 18/26 和 macOS 15/26 两个世代。
...
29. npm `1.0.0` bootstrap 建立 release lineage，首次受保护 tag 与四端协调版本为 `1.0.1`；新 repo 未迁移、保留或归档旧 tags。
30. Swift `1.0.1` 支持 iOS 18/26 与 macOS 15/26，并有对应 CI；后续按“仅最新两个 major generations”滚动提高 minimum deployment targets。
...
- [x] 不迁移或保留任何旧 tags；npm `1.0.0` bootstrap 建立新 lineage，首个受保护
  coordinated tag 为 `v1.0.1`。
...
- [x] 将 `1.0.1` minimum deployment targets 设为 iOS 18 和 macOS 15，并为 iOS 18/26、macOS 15/26 配置 CI。
...
- [x] 以 npm `1.0.0` bootstrap 建立新 lineage，并准备首个受保护 tag 与四端协调
  release `1.0.1`；确认新 repo 不包含旧 tags，且发布文档明确不承诺 C ABI compatibility。
...
1. 新 release lineage 由 npm `1.0.0` bootstrap 建立，首个受保护 tag 与四端协调
   release 为 `1.0.1`；不迁移、保留或归档旧 tags。
