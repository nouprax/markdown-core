...
- 新项目的 release lineage 由 npm `1.0.0` bootstrap 建立；受保护 `v1.0.1` 在产物构建前
  fail-closed 且未发布，首个四端协调 release 为 `1.0.2`。
- 新 repo 不迁移、不保留、不归档原 `cmark-gfm` 的旧 tags。
- 新 repo 的 tag namespace 只包含 Markdown Core 自身的 release attempts；失败的
  `v1.0.1` 保持不可变，首个协调版本为 `v1.0.2`。
...
- `1.0.2` 初始 minimum deployment targets 为 iOS 18 与 macOS 15，从而覆盖 iOS 18/26 和 macOS 15/26 两个世代。
...
29. npm `1.0.0` bootstrap 建立 release lineage；受保护 `v1.0.1` 在发布前失败并保持不可变，首个四端协调版本为 `1.0.2`；新 repo 未迁移、保留或归档旧 tags。
30. Swift `1.0.2` 支持 iOS 18/26 与 macOS 15/26，并有对应 CI；后续按“仅最新两个 major generations”滚动提高 minimum deployment targets。
...
- [x] 不迁移或保留任何旧 tags；npm `1.0.0` bootstrap 建立新 lineage；受保护
  `v1.0.1` 在发布前失败并保持不可变，首个 coordinated tag 为 `v1.0.2`。
...
- [x] 将 `1.0.2` minimum deployment targets 设为 iOS 18 和 macOS 15，并为 iOS 18/26、macOS 15/26 配置 CI。
...
- [x] 以 npm `1.0.0` bootstrap 建立新 lineage；保留发布前失败的受保护 `v1.0.1`，并准备
  首个四端协调 release `1.0.2`；确认新 repo 不包含旧 tags，且发布文档明确不承诺 C ABI compatibility。
- [x] 对齐 C、SwiftPM、Maven 和 npm 版本，并以根 `VERSION` 拒绝 artifact version 漂移。
- [x] 在 AGP 9.3 stable 发布后升级并重新验证 Gradle/Kotlin/Android compatibility matrix；cache-cold model、Android host/device tests、publication 与 consumer checks 必须在 `--warning-mode=fail` 下通过，确认 AGP 9.2.1 的上游 Project dependency notation warning 已消失，不得以 9.3 preview 作为首发工具链。
...
- [x] 在 Android Studio Quail 2 2026.1.2 或等价 IntelliJ IDEA 2026.1 执行一次 release
  clean-import smoke test，确认 KMP source sets 可见，且根 Gradle `allKotlinTests` task 可从
  IDE 运行；shared `All Kotlin tests` 仅为该 task 的平台无关快捷入口，不是 sample app；
  两个使用同一 JetBrains Gradle/KMP importer 的 IDE 不重复验收。
...
- [x] 从 clean checkout 运行全量 build/test/conformance/consumer checks。
...
1. 新 release lineage 由 npm `1.0.0` bootstrap 建立；受保护 `v1.0.1` 在发布前失败并保持
   不可变，首个四端协调 release 为 `1.0.2`；不迁移、保留或归档旧 tags。
