...
mode=${1:?usage: create-kotlin-product.sh linux-release|macos-native OUTPUT_DIRECTORY}
output=${2:?usage: create-kotlin-product.sh linux-release|macos-native OUTPUT_DIRECTORY}
...
"$root/scripts/release/stage-maven-publications.sh" "$stage" "$mode"
rm -rf "$output"
mkdir -p "$output"
