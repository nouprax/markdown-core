...
bash "$root/scripts/audit/check-test-topology.sh" "$root/$build_dir" "$configuration"
if find "$root/$build_dir" -type f \( -name '*stage_runner' -o -name '*stage_runner.exe' \) -print -quit | grep -q .; then
