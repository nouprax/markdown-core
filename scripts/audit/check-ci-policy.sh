...
scripts/audit/check-toolchain-versions.sh
...
test -x scripts/benchmark/run.mjs
grep -Fq 'name: Benchmark' "$benchmark"
grep -Fq 'node scripts/benchmark/run.mjs' "$benchmark"
grep -Fq 'install --no-install-recommends --yes valgrind' "$benchmark"
grep -Fq 'scripts/tooling/setup-environment.sh --install oracle-cmark' "$benchmark"
...
grep -Fq 'receiptBytes !== document.bytes' scripts/benchmark/run.mjs
for boundary in \
    ...
    grep -Fq "$boundary" scripts/benchmark/run.mjs || {
...
grep -Fq 'CMAKE_C_FLAGS_RELEASE' scripts/benchmark/run.mjs
grep -Fq 'verifyStageSymbols' scripts/benchmark/run.mjs
...
benchmark_build_description=(scripts/benchmark/run.mjs scripts/benchmark/compile-identity.mjs)
for cached in \
    'CMAKE_C_FLAGS' \
...
for inherited in CFLAGS LDFLAGS; do
    grep -Fq "process.env.$inherited" scripts/benchmark/run.mjs || {
...
for probe in '--help=target' '--help=params' '--help=common'; do
    grep -Fq -- "$probe" scripts/benchmark/run.mjs || {
...
grep -Fq 'refuseResponseFiles' scripts/benchmark/run.mjs || {
...
grep -Fq 'compilerConfiguration' scripts/benchmark/run.mjs || {
...
grep -Fq 'function agreed(' scripts/benchmark/run.mjs || {
...
if grep -Eq '\breturn "unknown"|summary: "unknown"' scripts/benchmark/run.mjs; then
...
grep -Fq 'measurementEnvironment' scripts/benchmark/run.mjs || {
...
if grep -Fq '...process.env' scripts/benchmark/run.mjs; then
...
grep -Fq 'buildEnvironment' scripts/benchmark/run.mjs || {
...
# driver's own.
grep -Fq 'measurementRoot' scripts/benchmark/run.mjs || {
...
grep -Fq -- '--untracked-files=all' scripts/benchmark/run.mjs || {
...
grep -Fq 'dispatchIdentity' scripts/benchmark/run.mjs || {
...
grep -Fq 'profilerBinaries' scripts/benchmark/run.mjs || {
...
grep -Fq 'compilerBinaries' scripts/benchmark/run.mjs || {
...
grep -Fq 'loadedLibraries' scripts/benchmark/run.mjs || {
...
grep -Fq 'corpusDigest' scripts/benchmark/run.mjs || {
...
grep -Fq 'realPath' scripts/benchmark/run.mjs || {
...
grep -Fq 'presetBinaryDir' scripts/benchmark/run.mjs || {
    echo "the stage benchmark does not take the profile build tree from the preset" >&2
    exit 1
}
if grep -Eq '"build/benchmark"|build/benchmark[^-]' scripts/benchmark/run.mjs; then
...
if grep -Eq 'skip-build|options\.build' scripts/benchmark/run.mjs; then
...
# bytes the tools run.
grep -q 'EMSCRIPTEN_COMMIT=[0-9a-f]\{40\}' scripts/tooling/setup-environment.sh || {
    echo "setup-environment.sh must pin the emsdk commit" >&2
    exit 1
}
grep -Fq 'scripts/tooling/install-clang-format.sh' scripts/tooling/setup-environment.sh || {
    echo "setup-environment.sh must use the repository clang-format installer" >&2
    exit 1
}
grep -q -- '--require-hashes' scripts/tooling/install-clang-format.sh || {
    echo "install-clang-format.sh must install from hash-locked requirements" >&2
    exit 1
}
grep -Fq 'scripts/tooling/install-clang-format.sh' "$ci" || {
    echo "CI must use the repository clang-format installer" >&2
    exit 1
}
grep -q -- '--require-hashes' scripts/tooling/format-cmake.sh || {
...
for adapter in \
    ...
    kotlin-host \
    swift; do
    test -x "scripts/build/create-${adapter}-tests.sh"
    test -x "scripts/shared/run-${adapter}-test-artifact.sh"
done
test -x scripts/build/create-kotlin-android-tests.sh
test -x scripts/shared/run-kotlin-android-test-artifact.sh
test -x scripts/tooling/prepare-swift-ios-simulator.sh
for platform in c es kotlin swift; do
    test -x "scripts/build/create-${platform}-product.sh"
done
grep -Fq -- '-DMARKDOWN_CORE_TESTS=OFF' scripts/build/create-c-product.sh
grep -Fq -- '-DMARKDOWN_CORE_TESTS=ON' scripts/build/create-c-tests.sh
grep -Fq 'Total Tests: [1-9][0-9]*' scripts/build/create-c-tests.sh
grep -Fq 'check-test-topology.sh" "$root/$build_dir" "$configuration"' scripts/build/create-c-tests.sh
grep -Fq "tr -d '\\r'" scripts/audit/check-test-topology.sh
if grep -Eq 'cmake --preset|cmake --build|swift test|run-gradle\.sh' scripts/audit/check-test-topology.sh; then
    echo "repository topology health-check must not duplicate a platform build or test discovery" >&2
    exit 1
fi
grep -Fq -- 'swift build --target MarkdownCore' scripts/build/create-swift-product.sh
grep -Fq -- '-DMARKDOWN_CORE_TESTS=OFF' scripts/release/stage-c.sh
grep -Fq 'Package.release.swift' scripts/release/check-swift-source-archive.sh
if grep -Eq 'swift package archive-source|cp .*Tests|cp .*Benchmarks|swift test|specs/canonical-ast' \
    scripts/release/check-swift-source-archive.sh; then
...
grep -Fq 'auditProductArchive' scripts/audit/check-maven-publications.mjs
grep -Fq 'publishes a test framework dependency' scripts/audit/check-maven-publications.mjs
...
grep -Fq ':packages:kotlin-markdown-core:checkKotlinAbi' scripts/release/stage-maven-publications.sh
grep -Fq '"${abi_tasks[@]}"' scripts/release/stage-maven-publications.sh
scripts/tooling/run-gradle.sh --quiet :packages:kotlin-markdown-core:tasks --all |
    grep -Fq checkKotlinAbi ||
    {
        echo "stage-maven-publications.sh names checkKotlinAbi and the build defines no such task" >&2
        exit 1
    }
if grep -Eq 'bundle-conformance|run-tests|run-conformance' scripts/build/create-es-product.sh; then
...
search 'test-without-building' scripts/shared/run-swift-test-artifact.sh
search -- '--skip-build' scripts/shared/run-swift-test-artifact.sh
grep -Fq -- '--scratch-path "$root_scratch"' scripts/build/create-swift-tests.sh
grep -Fq -- '--scratch-path "$root_scratch"' scripts/shared/run-swift-test-artifact.sh
grep -Fq 'remove_directory "$root/$build_dir"' scripts/build/create-c-tests.sh
grep -Fq 'rm -rf "$project_build/ci-test-artifact"' scripts/build/create-kotlin-host-tests.sh
for producer in scripts/build/create-c-tests.sh scripts/build/create-kotlin-host-tests.sh scripts/build/create-swift-tests.sh; do
...
if grep -Eq 'node-benchmark|macos-benchmark|stageJvmBenchmarkArtifact|jvm-benchmark|ci-benchmark' \
    scripts/build/create-swift-tests.sh \
    scripts/shared/run-swift-test-artifact.sh \
    scripts/build/create-kotlin-host-tests.sh \
    scripts/shared/run-kotlin-host-test-artifact.sh \
    scripts/shared/run-es-test-artifact.sh \
    scripts/shared/run-c-test-artifact.sh; then
    echo "CI test artifacts still route a removed benchmark payload" >&2
    exit 1
fi
grep -Fq -- "-destination 'generic/platform=iOS Simulator'" scripts/build/create-swift-tests.sh
grep -Fq 'prepare-swift-ios-simulator.sh' scripts/shared/run-swift-test-artifact.sh
if grep -Eq 'name=iPhone|OS=latest' scripts/build/create-swift-tests.sh scripts/shared/run-swift-test-artifact.sh \
    scripts/shared/run-swift-ios-tests.sh package.json; then
...
search 'artifact_verify ' scripts/shared/run-kotlin-android-test-artifact.sh
if search 'run-gradle\.sh|cmake --build|swift build|xcodebuild build-for-testing|build\.mjs|\bemcc\b' \
    scripts/shared/run-*-test-artifact.sh; then
...
grep -Fq 'scripts/tooling/setup-environment.sh --install oracle-cmark oracle-cmark-gfm' <<<"$oracle_job"
...
grep -Fq 'manage-central.sh upload build/markdown-core-maven-central.zip' <<<"$maven_stage_job"
if search 'manage-central\.sh upload' <(job_body maven-assemble "$release"); then
...
search 'publishingType=USER_MANAGED' scripts/release/manage-central.sh
for secret in \
...
search 'check-maven-publications\.mjs' "$release_dry_run"
