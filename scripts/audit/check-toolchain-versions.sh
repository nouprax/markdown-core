...
expect_equal "SwiftLint installer" "$swiftlint_version" \
    "$(sed -n 's/^VERSION="\([^"]*\)"/\1/p' scripts/tooling/install-swiftlint.sh)"
expect_equal "SwiftLint runner" "$swiftlint_version" \
    "$(sed -n 's/^VERSION="\([^"]*\)"/\1/p' scripts/tooling/lint-swift.sh)"
expect_equal "clang-format installer" "$clang_format_version" \
    "$(sed -n 's/^VERSION="\([^"]*\)"/\1/p' scripts/tooling/install-clang-format.sh)"
expect_equal "clang-format runner" "$clang_format_version" \
    "$(sed -n 's/^EXPECTED_VERSION="\([^"]*\)"/\1/p' scripts/tooling/format-c.sh)"
