 ...
 *
 *   node scripts/correctness/check-upstream-parity.mjs --oracle commonmark|gfm
 *                                          [--limit N] [--verbose]
 */
...
const ORACLES = {
    commonmark: {
        ...
        install: "scripts/tooling/setup-environment.sh --install oracle-cmark",
        label: "cmark",
        extensions: []
    },
    gfm: {
        ...
        install: "scripts/tooling/setup-environment.sh --install oracle-cmark-gfm",
        label: "cmark-gfm",
...
// scripts/correctness/fuzz-parity.mjs, which feeds generated inputs through this gate
...
if (unmappedKinds.size) {
    ...
    process.stderr.write(
        `\nupstream parity FAILED: ${String(unmappedKinds.size)} node kind(s) have no mapping: ` +
            `${[...unmappedKinds].join(", ")}\n` +
            "Add them to scripts/shared/upstream-cmark.mjs; an unmapped kind compares equal to itself and proves nothing.\n"
    );
    process.exit(1);
}
