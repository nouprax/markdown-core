 ...
 *
 *   node scripts/correctness/check-mdast-parity.mjs [--verbose]
 */
...
// scripts/correctness/check-upstream-parity.mjs.
...
if (unmapped.size) {
    process.stderr.write(
        `\nmdast parity FAILED: ${String(unmapped.size)} node kind(s) have no mapping: ${[...unmapped].join(", ")}\n` +
            "Add them to scripts/correctness/mdast-oracle.mjs; an unmapped kind compares equal to itself and proves nothing.\n"
    );
    process.exit(1);
}
