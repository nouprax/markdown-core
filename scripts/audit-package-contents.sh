...
if unzip -Z1 "$kotlin_jvm_jar" | grep -E '(^|/)(NativeBridge[^/]*|JvmNative|Walker|MarkupWalker|WalkingVisitor|WalkingVisitorKt)(\$[^/]*)?\.class$'; then
...
if unzip -Z1 "$kotlin_jvm_sources" | grep -E '(^|/)(NativeBridge[^/]*|Walker|MarkupWalker|WalkingVisitor|WireDecoder|WireKind|WireMarkupDecoder)[^/]*\.kt$|^commonMain/.*/(walker|wire)/|(^|/)native-bridge(/|$)'; then
...
if ! unzip -Z1 "$kotlin_jvm_sources" \
    | grep -qx 'commonMain/com/nouprax/markdown/core/visitor/Visitor.kt'; then
    echo "Kotlin JVM source publication is missing the unified visitor" >&2
    exit 1
fi
