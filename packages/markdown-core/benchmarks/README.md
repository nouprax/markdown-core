...
The single `pairs` registry in `corpus.json` distinguishes registered domain
proofs from candidates with explicit pending obligations. All 30 former pairs
remain measured candidates. The new `pair-insertion-dialect` /
`pair-insertion-common` workload exercises the `insertion-strong-v1` proof,
including nested and sibling spans. Its domain is deliberately stated; it does
not claim that arbitrary `+` and `*` runs are interchangeable.
...
or enter equivalent-work medians. `contract.pending` records the missing
obligation for every candidate, and does not mean the grammar is unpairable.
...
mapping is pending; its old count witness no longer grants Same-job status.
...
a candidate until its operation mapping is proved.
...
- every `with` reaches every state the split names and no `without` reaches
  ...
  span pair carries an explicit pending field-mapping obligation;
