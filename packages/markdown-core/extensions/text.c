...
static markdown_core_node *handle_backslash(markdown_core_parser *parser, subject *inline_parser) {
    ...
    unsigned char nextchar = markdown_core_inline_peek_char(inline_parser);
    if (nextchar == ' ') {
        ...
        bufsize_t end = inline_parser->pos;
        while (end < inline_parser->input.len &&
               !markdown_core_is_line_end(markdown_core_inline_peek_at(inline_parser, end)) &&
               markdown_core_isspace(markdown_core_inline_peek_at(inline_parser, end))) {
            end++;
            parser->whitespace_work++;
        }
        if ((end == inline_parser->input.len && !MARKDOWN_CORE_NODE_TYPE_INLINE_P(inline_parser->owner->kind)) ||
            (end < inline_parser->input.len &&
             markdown_core_is_line_end(markdown_core_inline_peek_at(inline_parser, end)))) {
            return make_str(inline_parser, start, start, markdown_core_chunk_literal("\\"));
        }
        advance(inline_parser);
        markdown_core_node *escaped =
            make_str(inline_parser, start, inline_parser->pos - 1, markdown_core_chunk_literal("\\ "));
        if (escaped) {
    ...
    if ((parser->backslash_ispunct ? parser->backslash_ispunct : markdown_core_ispunct)(nextchar)) {
        if (nextchar == '\\' && !any_extension_dispatches(parser, '\\')) {
            bufsize_t end = start;
            while (end + 1 < inline_parser->input.len && inline_parser->input.data[end] == '\\' &&
                   inline_parser->input.data[end + 1] == '\\') {
                end += 2;
            }
            if (end - start >= 4) {
                bufsize_t output_len = (end - start) / 2;
                unsigned char *output = (unsigned char *)inline_parser->mem->calloc((size_t)output_len + 1, 1);
                if (output) {
                    ...
                    run = make_str(inline_parser, start, end - 1, contents);
        ...
        {
            markdown_core_node *escaped =
                make_str(inline_parser, inline_parser->pos - 2, inline_parser->pos - 1,
                         markdown_core_chunk_dup(&inline_parser->input, inline_parser->pos - 1, 1));
            return escaped;
        }
    } else if (!markdown_core_inline_is_eof(inline_parser) && markdown_core_inline_skip_line_end(inline_parser)) {
        markdown_core_inline_push_boundary(inline_parser, inline_parser->pos);
        ...
        markdown_core_node *hard = markdown_core_inline_make_simple_subj(inline_parser, MARKDOWN_CORE_NODE_LINE_BREAK);
        if (hard) {
            /* The break's extent is the backslash and the line ending it
             * escapes; `inline_parser->pos` is one past that ending's last byte, CR, LF
             * or CRLF alike. Projected from the two offsets, so the frame
             * captured before the consume is only the fallback's. */
            markdown_core_inline_parser_place(inline_parser, hard, start, inline_parser->pos - 1);
        }
        return hard;
    } else {
...
static markdown_core_node *handle_entity(subject *inline_parser) {
    markdown_core_strbuf ent = MARKDOWN_CORE_BUF_INIT(inline_parser->mem);
    bufsize_t len;

    advance(inline_parser);

    len = houdini_unescape_ent(&ent, inline_parser->input.data + inline_parser->pos,
    ...
    inline_parser->pos += len;
    if (ent.oom) {
        inline_parser->oom = 1;
    }
    return make_str(inline_parser, inline_parser->pos - 1 - len, inline_parser->pos - 1,
...
static markdown_core_node *match(const markdown_core_extension *self, markdown_core_parser *parser,
    ...
    if (character == '\\') {
        return handle_backslash(parser, inline_parser);
    }
    if (character == '&') {
        return handle_entity(inline_parser);
    }
    return NULL;
}
markdown_core_node *markdown_core_text_parse(markdown_core_parser *parser, subject *inline_parser, bufsize_t endpos) {
    ...
    for (bufsize_t at = inline_parser->pos; at < endpos;) {
        int32_t scalar = 0;
        int width = markdown_core_utf8proc_iterate(inline_parser->input.data + at, endpos - at, &scalar);
        parser->whitespace_work++;
        if (markdown_core_utf8proc_is_space(scalar)) {
            markdown_core_inline_push_boundary(inline_parser, at + width);
        }
        at += width > 0 ? width : 1;
    }
    ...
    markdown_core_media_record_text(parser, inline_parser, endpos);
    contents = markdown_core_chunk_dup(&inline_parser->input, inline_parser->pos, endpos - inline_parser->pos);
    startpos = inline_parser->pos;
    inline_parser->pos = endpos;

    // if we're at a newline, strip trailing spaces.
    if (markdown_core_is_line_end(markdown_core_inline_peek_char(inline_parser))) {
        markdown_core_chunk_rtrim(&contents);
    }

    markdown_core_node *new_inl = make_str(inline_parser, startpos, endpos - 1, contents);
