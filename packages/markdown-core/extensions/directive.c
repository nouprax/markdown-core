...
static void set_chunk_bytes(markdown_core_mem *mem, markdown_core_chunk *chunk, const unsigned char *data,
                            bufsize_t len) {
    markdown_core_chunk_free(mem, chunk);
    chunk->data = (unsigned char *)data;
    chunk->len = len;
    chunk->alloc = 0;
    markdown_core_chunk_to_cstr(mem, chunk);
}
...
static int append_attribute(markdown_core_mem *mem, directive_attribute **head, directive_attribute **tail,
                            ...
                            bufsize_t value_len, size_t index) {
    ...
    *result = NULL;
    ...
    while (pos < len) {
        ...
        if (!parse_json_string(mem, data, len, &pos, &value))
            goto member_done;
        if (!append_attribute(mem, &head, &tail, name.data, name.len, value.data, value.len, count++))
            goto member_done;
...
int markdown_core_extensions_set_directive_name(markdown_core_node *node, const char *name) {
    ...
    markdown_core_chunk_set_cstr(markdown_core_node_mem(node), &directive->name, name);
    return 1;
}
...
static int parse_directive_suffix(markdown_core_mem *mem, unsigned char *data, bufsize_t len, bufsize_t pos,
    ...
    if (pos < len && data[pos] == '{') {
        ...
        if (!scan_attributes_raw(data, len, pos, &attr_start, &attr_len, &pos) ||
            !parse_attributes(mem, data + attr_start, attr_len, &parsed->attributes))
            return 0;
    }
...
static int apply_parsed_directive(markdown_core_syntax_extension *extension, markdown_core_node *node,
    ...
    set_chunk_bytes(mem, &directive->name, data + parsed->name_start, parsed->name_len);
...
static int scan_parsed_attributes(markdown_core_mem *mem, const unsigned char *data, bufsize_t len, bufsize_t pos,
    ...
    int valid = scan_attributes_raw(data, len, pos, &attr_start, &attr_len, end) &&
                parse_attributes(mem, data + attr_start, attr_len, &attributes);
...
static markdown_core_node *match_colon_directive(markdown_core_syntax_extension *extension,
    ...
    if (pos < chunk->len && chunk->data[pos] == '{') {
        ...
        if (!scan_attributes_raw(chunk->data, chunk->len, pos, &attr_start, &attr_len, &end) ||
            !parse_attributes(parser->mem, chunk->data + attr_start, attr_len, &attributes))
            return NULL;
...
static markdown_core_node *match_label_closer(markdown_core_parser *parser, markdown_core_inline_parser *inline_parser,
    ...
    if (offset + 1 < chunk->len && chunk->data[offset + 1] == '{' &&
        scan_parsed_attributes(parser->mem, chunk->data, chunk->len, offset + 1, &end)) {
...
static int set_attributes_from_wrapper(markdown_core_node *node, const unsigned char *data, bufsize_t len,
    ...
    if (!directive || !scan_attributes_raw(data, len, pos, &attr_start, &attr_len, &end) || end != len ||
        !parse_attributes(markdown_core_node_mem(node), data + attr_start, attr_len, &attributes))
        return 0;
...
static delimiter *insert_label_directive(markdown_core_syntax_extension *extension, markdown_core_parser *parser,
    ...
    if (closer_literal->len > 1 && closer_literal->data[1] == '{' &&
        !set_attributes_from_wrapper(directive_node, closer_literal->data, closer_literal->len, 1)) {
