 ...
 * Every byte is inspected at most twice, including failed and unclosed forms. */
static markdown_core_node *match(const markdown_core_extension *extension, markdown_core_parser *parser,
    ...
    node = markdown_core_node_new_with_mem_and_ext(MARKDOWN_CORE_NODE_CROSS_LINK, parser->mem, extension);
    if (!node) {
        parser->oom = true;
        return NULL;
    }
