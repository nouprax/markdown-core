...
static void S_strip_html_comments(markdown_core_node *root) {
    bool stripped = false;
    ...
    if (stripped) {
        markdown_core_consolidate_text_nodes(root);
    }
}
...
int markdown_core_parser_attach_syntax_extension(markdown_core_parser *parser,
    ...
    parser->syntax_extensions = markdown_core_llist_append(parser->mem, parser->syntax_extensions, extension);
    if (extension->match_inline || extension->insert_inline_from_delim) {
        parser->inline_syntax_extensions =
            markdown_core_llist_append(parser->mem, parser->inline_syntax_extensions, extension);
    }
...
static bufsize_t parse_list_marker(markdown_core_mem *mem, markdown_core_chunk *input, bufsize_t pos,
    ...
    while (cont_type != MARKDOWN_CORE_NODE_CODE_BLOCK && cont_type != MARKDOWN_CORE_NODE_HTML_BLOCK &&
        ...
        if (!indented && peek_at(input, parser->first_nonspace) == '>') {
            ...
            *container = add_child(parser, *container, MARKDOWN_CORE_NODE_THEMATIC_BREAK, parser->first_nonspace + 1);
            ...
            markdown_core_chunk_to_cstr(parser->mem, &c);
            ...
            *container = add_child(parser, *container, MARKDOWN_CORE_NODE_FOOTNOTE_DEFINITION,
            ...
            (*container)->internal_offset = matched;
                   ...
                   (matched = parse_list_marker(parser->mem, input, parser->first_nonspace,
...
markdown_core_node *markdown_core_parser_finish(markdown_core_parser *parser) {
    ...
    markdown_core_consolidate_text_nodes(parser->root);
