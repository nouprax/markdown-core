...
export class Document {
    ...
    constructor(markdown, options) {
        ...
        if (!this.#native)
            throw new ParseError("allocationFailed", "failed to allocate a native session");
        if (initial === undefined)
            return;
        const bytes = typeof initial === "string" ? utf8Encoder.encode(initial) : initial;
        const session = this.#native;
        withHeapBytes(bytes, (chunkPointer) => discardOut((output) => native.es_session_advance(session, chunkPointer, bytes.length, output, output + 4)));
    }
    /**
    ...
    feed(chunk) {
        ...
        if (!(bytes instanceof Uint8Array))
            throw new TypeError("chunk must be a string or a Uint8Array");
        const session = this.#live();
        return withHeapBytes(bytes, (chunkPointer) => copyOut((output) => native.es_session_feed(session, chunkPointer, bytes.length, output, output + 4)));
    }
    /**
    ...
    #live() {
        if (!this.#native)
            throw new Error("the document is sealed or disposed");
        return this.#native;
    }
}
