 ...
 */
export function withHeapBytes(bytes, action) {
 ...
 */
export function copyOut(invoke) {
 ...
 */
export function discardOut(invoke) {
...
function takePayload(invoke) {
    ...
    try {
        ...
        if (payloadPointer)
            native.es_wire_free(payloadPointer);
        native.free(output);
    }
}
/**
 ...
 */
function makeRead(semantic, concrete) {
    const read = {
        semantic,
        concrete
    };
 ...
 * about to read them. */
export function optionsMask(parseOptions) {
    ...
    for (const option of options) {
        ...
        if (typeof value !== "boolean")
            throw new TypeError(`${option.name} must be a boolean`);
        if (value)
            flags |= option.mask;
    }
    return flags;
