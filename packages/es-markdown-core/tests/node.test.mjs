...
test("api: options gate extensions", () => {
    const markdown = "| a |\n| --- |\n| b |\n";
    assert.equal(Document.parse(markdown).semantic.content[0].kind, "table");
    assert.equal(Document.parse(markdown, { tables: false }).semantic.content[0].kind, "paragraph");
});
...
test("errors: empty input is valid and arguments are checked", () => {
    assert.deepEqual(Document.parse("").semantic.content, []);
...
test("robustness: large documents copy completely before native release", () => {
    ...
    assert.equal(Document.parse(unit.repeat(5_000)).semantic.content.length, 10_000);
});
...
test("concrete: the view is total and its owners resolve after native release", () => {
    ...
    const concrete = parsed.concrete;
    ...
    for (let index = 0; index < concrete.regionCount; index += 1) {
        ...
        assert.ok(parsed.ownerOf(region) !== undefined);
    ...
    assert.ok(markers > 0);
    assert.equal(parsed.ownerOf({ start: 0, length: 1, role: RegionRole.content, owner: [99] }), undefined);
    ...
    assert.deepEqual(ownerAt(parsed, 42).scope.start, { line: 5, column: 3 });
    assert.deepEqual(ownerAt(parsed, 106).scope.start, { line: 10, column: 1 });
...
function ownerAt(parsed, offset) {
    for (let index = 0; index < parsed.concrete.regionCount; index += 1) {
        const region = parsed.concrete.region(index);
        if (offset >= region.start && offset < region.start + region.length) {
            return parsed.ownerOf(region);
        }
    }
