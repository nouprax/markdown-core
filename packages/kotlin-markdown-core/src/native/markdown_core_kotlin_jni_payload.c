...
static void write_node(jni_payload_buffer *buffer, jni_payload_stack *stack, jni_payload_resources *resources,
    ...
    case MARKDOWN_CORE_KIND_LIST: {
        ...
        bool tight = false;
        if (!markdown_core_node_list_properties(node, &flavor, &start, &style, &delimiter, &tight)) {
            buffer->failure = JNI_PAYLOAD_INTERNAL;
            return;
        }
        ...
        put_i32(buffer, start.has_value ? (int32_t)style : 0);
        put_i32(buffer, start.has_value ? (int32_t)delimiter : 0);
