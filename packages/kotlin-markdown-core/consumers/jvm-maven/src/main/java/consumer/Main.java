...
import com.nouprax.markdown.core.DocumentRoot;
...
public final class Main {
    ...
    public static void main(String[] args) {
        ...
        Document parsed = Document.Companion.parse("héllo 🚀\n", options);
        DocumentRoot document = parsed.getSemantic();
        if (document.getContent().size() != 1) {
            throw new IllegalStateException("Document.parse returned unexpected top-level content");
        }
        if (parsed.getConcrete().getSource().length != "héllo 🚀\n".getBytes().length) {
